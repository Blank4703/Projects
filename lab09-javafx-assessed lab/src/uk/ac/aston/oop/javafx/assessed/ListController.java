...
public class ListController {
	...
	public void addPressed(){
		...
		final CreateVideoController controller = new CreateVideoController(model);
		loader.setController(controller);
		try{
			VBox root = loader.load();
			final Stage video = new Stage();
			video.setTitle("Create Video");
			video.initModality(Modality.APPLICATION_MODAL);
			video.setScene(new Scene(root, 700, 500));
			video.showAndWait();
